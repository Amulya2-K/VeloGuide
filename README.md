...
VeloGuide uses JWT-based authentication with role-based authorization.
...
Token sent to frontend
  │
  ▼
Protected requests include JWT
  │
  ▼
...
Travelers can search for guides based on their selected destination.
...
Traveler selects destination
      ...
      Normalize city name
       ...
       Search guide cache
             │
             ▼
    Find guides for that city
             │
             ▼
       Display guide profiles
```
...
Guide Cache
    ↓
Guides registered for Mumbai
```

The normalized city key ensures consistent city-based searching even when users enter different capitalization or spacing.
...
The backend sends these preferences to the Google Gemini API.
...
The generated itinerary is tailored according to the destination, duration, interests, and travel preferences provided by the traveler.

---

## 🔄 Booking Workflow

VeloGuide provides a structured workflow between travelers and guides.

```text
Traveler selects guide
        │
        ▼
Booking created
        │
...
Socket.IO can be used to communicate booking status changes to connected users in real time.
...
A MERN-based travel platform integrating location-aware guide discovery, AI-assisted itinerary planning, booking management, intelligent caching, and real-time communication.

### Built With

**React.js • Node.js • Express.js • MongoDB • Mongoose • JWT • BcryptJS • Google Gemini API • Socket.IO**